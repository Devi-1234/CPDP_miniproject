...
class CORAL:
    def fit_transform(self, X_source, X_target):
        ...
        source_whitened = np.dot(X_source, np.linalg.inv(np.linalg.cholesky(cov_source)))
        X_source_aligned = np.dot(source_whitened, np.linalg.cholesky(cov_target))

        return X_source_aligned, X_target

def run_coral(source_files, target_filename, test_filename, data_path, ml_model):
    ...
    coral = CORAL()
    X_source_coral, X_target_coral = coral.fit_transform(X_source_bal, X_target_scaled)
    X_test_coral = coral.fit_transform(X_target_scaled, X_test_scaled)[1]
    ...
    model.fit(X_source_coral, y_source_bal)

    # Get probabilities
    y_prob = model.predict_proba(X_test_coral)[:, 1]
    
    # Make predictions
    y_pred = model.predict(X_test_coral)
