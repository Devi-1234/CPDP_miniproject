...
def mmd_rbf(X, Y, gamma=1.0):
    ...
    K_XX = np.exp(-gamma * (c(X_sqnorms) + r(X_sqnorms) - 2 * XX))
    K_XY = np.exp(-gamma * (c(X_sqnorms) + r(Y_sqnorms) - 2 * XY))
    K_YY = np.exp(-gamma * (c(Y_sqnorms) + r(Y_sqnorms) - 2 * YY))

    return np.mean(K_XX) - 2 * np.mean(K_XY) + np.mean(K_YY)

def run_mmd(source_files, target_filename, test_filename, data_path, ml_model):
    ...
    print("Applying MMD...")
    gamma = 1.0
    mmd_value = mmd_rbf(X_source_bal, X_target_scaled, gamma)
